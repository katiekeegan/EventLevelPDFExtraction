...
def create_minimal_simulators():
    ...
    class MinimalRealisticDIS:
        ...
        def sample(self, theta, nevents=1000):
            ...
            shape = x.clamp(min=1e-6, max=1.0)**a * (1 - x.clamp(min=0.0, max=1.0))**b
            poly = 1 + c * x + d * x**2
    ...
    class MinimalMCEGSimulator:
        ...
        def sample(self, theta, nevents=1000):
            ...
            theta_expanded = theta[:4] if len(theta) >= 4 else torch.cat([theta, torch.zeros(4-len(theta), device=self.device)])
            f2 = x * (theta_expanded[0] * x**theta_expanded[1] + theta_expanded[2] * x**theta_expanded[3])
            return torch.stack([x, Q2], dim=1)
    
    return MinimalGaussian2DSimulator, MinimalSimplifiedDIS, MinimalRealisticDIS, MinimalMCEGSimulator
...
def get_simulators_and_utils():
    ...
    if FULL_SIMULATORS_AVAILABLE:
        return (Gaussian2DSimulator, SimplifiedDIS, RealisticDIS, MCEGSimulator, 
                log_feature_engineering, advanced_feature_engineering)
    else:
        minimal_sims = create_minimal_simulators()
        return (*minimal_sims, minimal_log_feature_engineering, minimal_advanced_feature_engineering)
...
def generate_data_for_problem(problem, num_samples, num_events, n_repeat, device, output_dir):
    """Generate and save data for a specific problem."""
    
    # Get appropriate simulators and utilities
    Gaussian2DSim, SimplifiedDISSim, RealisticDISSim, MCEGSim, log_feat_eng, adv_feat_eng = get_simulators_and_utils()
    ...
    for i in tqdm(range(num_samples), desc=f"Generating {problem} data"):
        ...
        stacked_events = torch.stack(event_sets, dim=0)  # [n_repeat, num_events, feature_dim]
    ...
    all_events = torch.stack(all_event_data, dim=0)  # [num_samples, n_repeat, num_events, feature_dim]
...
def main():
    parser = argparse.ArgumentParser(description='Generate precomputed datasets for PDF Parameter Inference')
    parser.add_argument('--problems', nargs='+', default=['gaussian', 'simplified_dis'], 
    ...
    for problem in args.problems:
        try:
            filepath = generate_data_for_problem(
                problem, args.num_samples, args.num_events, 
                args.n_repeat, device, args.output_dir
            )
