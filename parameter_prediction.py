...
# Import wandb robustly; avoid accidental shadowing via wildcard imports later
try:
    import importlib
...
try:
    from precomputed_datasets import (DistributedPrecomputedDataset,
                                      PrecomputedDataset,
                                      create_precomputed_dataloader,
                                      filter_valid_precomputed_files)

    PRECOMPUTED_AVAILABLE = True
except ImportError:
    print(
        "Warning: precomputed_datasets.py not found. Precomputed data support disabled."
    )
...
def generate_precomputed_data_if_needed(
    problem, num_samples, num_events, n_repeat=2, output_dir="precomputed_data"
):
    """
    ...
    if os.path.exists(exact_file_path):
        ...
        #     # Generate new data
        print(
            f"Precomputed data not found for {problem} with ns={num_samples}, ne={num_events}, nr={n_repeat}"
        )
...
def train_joint(
    ...
    opt = optim.Adam(
        list(model.parameters()) + list(param_prediction_model.parameters()),
        lr=lr,
        weight_decay=1e-4,
    )
    ...
    for epoch in range(epochs):
        ...
        num_train_batches = 0
        # accumulators for per-dimension MSE (sum of squared errors per-dim)
        per_dim_sse_train = None
        ...
        for theta, x_sets in train_dataloader:
            ...
            if (
                hasattr(args, "check_training_bounds")
                and args.check_training_bounds
                and epoch == 0
                and num_train_batches == 0
            ):
                ...
                for i in range(theta_bounds.shape[0]):
                    ...
                    bound_min, bound_max = (
                        theta_min_cpu[i].item(),
                        theta_max_cpu[i].item(),
                    )

                    if min_val < bound_min or max_val > bound_max:
                        print(
                            f"   ⚠️  Parameter {i}: range=[{min_val:.3f}, {max_val:.3f}], bounds=[{bound_min:.1f}, {bound_max:.1f}]"
                        )
                    else:
                        print(
                            f"   ✅ Parameter {i}: range=[{min_val:.3f}, {max_val:.3f}], bounds=[{bound_min:.1f}, {bound_max:.1f}]"
                        )
                print()
            ...
            num_train_batches += 1
            # accumulate per-dim squared error for unnormalized loss analysis
            # raw_pred and theta are in original units and shape (B, D)
            with torch.no_grad():
                batch_sse = (
                    ((raw_pred - theta) ** 2).sum(dim=0).detach()
                )  # sum over batch for each dim -> (D,)
                if per_dim_sse_train is None:
                    per_dim_sse_train = batch_sse
                else:
                    per_dim_sse_train += batch_sse
                # track exact number of examples (handles repeat_interleave)
                total_train_examples += raw_pred.size(0)
        ...
        with torch.no_grad():
            for theta, x_sets in val_dataloader:
                ...
                num_val_batches += 1
                # accumulate per-dim SSE for validation
                batch_sse = ((raw_pred - theta) ** 2).sum(dim=0).detach()
                if per_dim_sse_val is None:
                    per_dim_sse_val = batch_sse
                else:
                    per_dim_sse_val += batch_sse
                total_val_examples += raw_pred.size(0)
        ...
        if rank == 0:
            if wandb_enabled:
                # compute per-dim MSE averages from accumulated SSE (divide by total samples seen)
                try:
                    if per_dim_sse_train is not None and total_train_examples > 0:
                        per_dim_mse_train = (
                            (per_dim_sse_train / float(total_train_examples))
                            .cpu()
                            .tolist()
                        )
                    else:
                        per_dim_mse_train = None
                    if per_dim_sse_val is not None and total_val_examples > 0:
                        per_dim_mse_val = (
                            (per_dim_sse_val / float(total_val_examples)).cpu().tolist()
                        )
                    else:
                        per_dim_mse_val = None
                    ...
                    if per_dim_mse_train is not None:
                        log_dict["train_mse_per_dim"] = per_dim_mse_train
                        # also log individual dims
                        for i, v in enumerate(per_dim_mse_train):
                            log_dict[f"train_mse_dim/{i}"] = v
                        # per-dim scalars are added above; W&B will create separate panels per scalar key
                    if per_dim_mse_val is not None:
                        log_dict["val_mse_per_dim"] = per_dim_mse_val
                        for i, v in enumerate(per_dim_mse_val):
                            log_dict[f"val_mse_dim/{i}"] = v
                        # per-dim scalars are added above; W&B will create separate panels per scalar key
            ...
            print(
                f"Epoch {epoch + 1:3d} | Train Loss: {avg_train_loss:.6f} | Val Loss: {avg_val_loss:.6f}"
            )
            ...
            if (epoch + 1) % save_every == 0 or (epoch + 1) == epochs:
                ...
                torch.save(
                    param_prediction_model.state_dict(),
                    os.path.join(output_dir, f"params_model_epoch_{epoch+1}.pth"),
                )
    if rank == 0:
        torch.save(model.state_dict(), os.path.join(output_dir, "final_model.pth"))
        torch.save(
            param_prediction_model.state_dict(),
            os.path.join(output_dir, "final_params_model.pth"),
        )
        ...
        try:
            ...
            lap_transformer.fit(
                train_dataloader
            )  # Use training dataloader for Laplace fitting
            ...
            try:
                ...
                def save_laplace(
                    ...
                    print(
                        f"Saving Laplace model to {os.path.join(output_dir, filename)}"
                    )
                    # Basic save if detailed save not available
                    torch.save(
                        lap_model.state_dict(), os.path.join(output_dir, filename)
                    )
...
def create_train_val_datasets(args, rank, world_size, device):
    ...
    if (
        hasattr(args, "use_precomputed")
        and args.use_precomputed
        ...
        try:
            ...
            print(
                f"Loaded precomputed datasets - Train: {metadata}, Val: {val_samples} samples"
            )
...
def main_worker(rank, world_size, args):
    ...
    if world_size > 1 and hasattr(args, "use_precomputed") and args.use_precomputed:
        ...
        if rank == 0:
            ...
            train_dataset, val_dataset, input_dim = create_train_val_datasets(
    ...
    if args.dataloader_workers > 0:
        ...
        print(
            f"      If you get 'Cannot re-initialize CUDA in forked subprocess' error,"
        )
        print(f"      try --dataloader_workers=0 to disable multiprocessing")
    else:
        print(
            f"   🔧 Using 0 workers (main process only) - avoids multiprocessing issues"
        )
    ...
    if rank == 0 and args.wandb:
        try:
            # Diagnostic prints to capture why wandb.init may be missing in single-GPU mode
            try:
                import inspect
                import sys

                print(f"wandb object: {wandb}")
                print(f"wandb type: {type(wandb)}")
                print(f"wandb module file: {getattr(wandb, '__file__', 'n/a')}")
                print(f"wandb hasattr init: {hasattr(wandb, 'init')}")
                print(f"sys.modules['wandb']: {sys.modules.get('wandb')}")
            except Exception as _diag:
                print(f"wandb diagnostics failed: {_diag}")

            if wandb is None or not hasattr(wandb, "init"):
                raise ImportError("wandb module not available or missing init")
            wandb.init(
                project="quantom_end_to_end",
                name=args.experiment_name,
                config=vars(args),
            )
        except Exception as e:
            print(f"⚠️ wandb initialization failed, disabling wandb: {e}")
            # disable wandb usage downstream
            args.wandb = False
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--val_samples", type=int, default=1000, help="Number of validation samples"
    )
    ...
    if args.single_gpu or world_size <= 1:
        ...
        try:
            ...
            if "Cannot re-initialize CUDA in forked subprocess" in str(e):
                ...
                print(
                    "   This error occurs when CUDA is initialized before multiprocessing fork."
                )
                print("   💡 SOLUTIONS:")
                print(
                    "   1. Set multiprocessing start method to 'spawn' at script start:"
                )
        ...
        if mp.get_start_method() != "spawn":
            print(
                f"   ⚠️  WARNING: Current method is not 'spawn' - may cause CUDA issues"
            )
        print()
        try:
            ...
            if "Cannot re-initialize CUDA in forked subprocess" in str(e):
                ...
                print(
                    "   This is likely due to CUDA being initialized before mp.spawn."
                )
                print("   💡 SOLUTIONS:")
                print(
                    "   1. Ensure multiprocessing start method is set to 'spawn' before any imports"
                )
