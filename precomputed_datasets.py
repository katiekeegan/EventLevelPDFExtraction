...
def filter_files_by_exact_parameters(
    ...
    exact_matches = []
    print(
        f"   🔍 FILTERING FOR EXACT PARAMETERS: ns={required_ns}, ne={required_ne}, nr={required_nr}"
    )
    ...
    print(
        f"   📊 EXACT MATCH RESULT: {len(exact_matches)} files match out of {len(file_list)} total"
    )
...
def filter_valid_precomputed_files(file_list: List[str]) -> List[str]:
    ...
    print(
        f"   📊 FILTERING RESULT: {len(valid_files)} valid out of {len(file_list)} total files"
    )
    return valid_files
...
class PrecomputedDataset(Dataset):
    ...
    def __init__(
        ...
        if os.path.isfile(data_dir):
            ...
            if exact_ns is not None or exact_ne is not None or exact_nr is not None:
                ...
                print(
                    f"   ✓ File matches exact parameters: ns={params['ns']}, ne={params['ne']}, nr={params['nr']}"
                )
        ...
        if os.path.isdir(data_dir) and (
            ...
            if not self.data_files:
                print(
                    f"   ✗ FAILURE: No files match exact parameters ns={exact_ns}, ne={exact_ne}, nr={exact_nr}"
                )
        ...
        if not self.data_files:
            if os.path.isdir(data_dir):
                if valid_files and (
                    ...
                    print(
                        f"   ✗ FAILURE: No files match exact parameters for problem '{problem}'"
                    )
                    ...
                    print(
                        f"   ⚠️  Warning: Found {len(temp_files)} temporary/incomplete files for problem '{problem}' in {data_dir}: {[os.path.basename(f) for f in temp_files]}"
                    )
                    print(
                        f"   📁 Only complete .npz files (without .tmp) are considered valid precomputed data."
                    )
                    print(
                        f"   💡 To fix: Remove '.tmp' from complete files or regenerate data without .tmp suffix"
                    )
                    print(
                        f"   ✗ FAILURE: No valid precomputed data files found for problem '{problem}' in {data_dir}"
                    )
                else:
                    # No files found for this problem at all
                    print(
                        f"   ✗ FAILURE: No precomputed data files found for problem '{problem}' in {data_dir}"
                    )
            print()
            raise FileNotFoundError(
        ...
        print(
            f"   ✓ SUCCESS: Found {len(self.data_files)} valid data files for {problem}"
        )
        if os.path.isdir(data_dir):
            if len(all_data_files) > len(self.data_files):
                ignored_files = [f for f in all_data_files if f not in self.data_files]
                print(
                    f"   🗑️  Ignored {len(ignored_files)} files: {[os.path.basename(f) for f in ignored_files]}"
                )
        print()
    ...
    def _load_data(self):
            ...
            try:
                ...
                if "problem" in data:
                    ...
                    if file_problem != self.problem:
                        print(
                            f"   ⚠️  WARNING: File {filename} is for problem '{file_problem}', expected '{self.problem}' - SKIPPING"
                        )
                        continue
                ...
                if params:
                    ...
                    print(
                        f"   ✓ Loaded {filename}: ns={params['ns']}, ne={params['ne']}, nr={params['nr']}"
                    )
                    print(
        ...
        if len(all_thetas) > 1:
            ...
            for i, (events, thetas) in enumerate(
                ...
                if thetas.shape[1:] != first_theta_shape[1:]:
                    print(
                        f"   ✗ INCOMPATIBLE: File {i} theta shape {thetas.shape} vs file 0 {first_theta_shape}"
                    )
                    compatible = False
                ...
                if events.shape[1:] != first_events_shape[1:]:
                    print(
                        f"   ✗ INCOMPATIBLE: File {i} events shape {events.shape} vs file 0 {first_events_shape}"
                    )
                    compatible = False
        ...
        print(
            f"   ✓ Final concatenated data: thetas {self.thetas.shape}, events {self.events.shape}"
        )
