...
def plot_PDF_distribution_single_CNF(
    ...
    latent = make_latent_from_true_params(
        simulator,
        pointnet_model,
        true_params.to(device),
        num_events=n_events_for_latent,
        device=device,
    ...
    x_lo, x_hi = x_range
    x_vals = (
        torch.logspace(np.log10(x_lo), np.log10(x_hi), nx, device=device)
        if x_lo > 0
        else torch.linspace(x_lo, x_hi, nx, device=device)
    )
    ...
    if problem == "simplified_dis":
        ...
        # Bands
        up_lo, up_mid, up_hi = function_bands_over_theta_samples(
            eval_up, theta_samples, x_vals, device=device
        )
        dn_lo, dn_mid, dn_hi = function_bands_over_theta_samples(
            eval_down, theta_samples, x_vals, device=device
        )
        ...
        for mid, lo, hi, truth, name, color in [
            ...
            ax.set_xlim(x_range)
            ax.set_xlabel(r"$x$")
            ax.set_ylabel(rf"${name}(x\mid\theta)$")
            ax.grid(True, which="both", linestyle=":", linewidth=0.5)
            ax.legend(frameon=False)
            plt.tight_layout()
        ...
        for Q2_fixed in Q2_slices:
            ...
            lo, mid, hi = function_bands_over_theta_samples(
                eval_q_fixed, theta_samples, x_vals, device=device
            )
            ...
            ax.plot(
                ...
                linestyle="--",
                linewidth=2,
                label=rf"Median $\hat{{q}}(x, Q^2={Q2_fixed})$",
            )
            ax.fill_between(
                x_vals.detach().cpu(),
                lo.detach().cpu(),
...
def evaluate_over_n_parameters_CNF(
    ...
    assert (
        feature_fn is not None
    ), "Pass your advanced_feature_engineering as feature_fn."
    ...
    for _ in range(n):
        ...
        if problem == "simplified_dis":
            ...
            chi2_up.append(
                compute_chisq_statistic(true_up.cpu().numpy(), pred_up.cpu().numpy())
            )
            chi2_down.append(
                compute_chisq_statistic(true_dn.cpu().numpy(), pred_dn.cpu().numpy())
            )
            ...
            for Q2_fixed in Q2_slices:
                ...
                chis.append(
                    compute_chisq_statistic(true_q.cpu().numpy(), pred_q.cpu().numpy())
                )
    ...
    if problem == "simplified_dis":
        ...
        plt.hist(
            chi2_down[chi2_down < chi2_down_clip], bins=50, alpha=0.6, label="Chi² Down"
        )
        plt.legend()
        ...
        print(
            "Stored mean/std χ² over Q² slices in chi2_up/chi2_down arrays (rename if desired)."
        )
...
def cnf_sample_theta_SimpleCNF(
    ...
    while remaining > 0:
        ...
        if B == 1:
            z = base_mean + base_std * torch.randn(
                m, theta_dim, device=device
            )  # [m, D]
...
def plot_uncertainty_decomposition_comparison(
    ...
    for i in range(n_params):
        ...
        bars = ax_top.bar(
            methods, uncertainties, color=colors, alpha=0.7, edgecolor="black"
        )
        ...
        for bar, val in zip(bars, uncertainties):
            ax_top.text(
                ...
                bar.get_height() + max(uncertainties) * 0.01,
                f"{val:.4f}",
                ha="center",
                va="bottom",
                fontsize=9,
        ...
        bars = ax_bottom.bar(
            methods, biases, color=colors, alpha=0.7, edgecolor="black"
        )
        ...
        for bar, val in zip(bars, biases):
            y_pos = (
                bar.get_height() + max(abs(min(biases)), max(biases)) * 0.01
                if val >= 0
                else bar.get_height() - max(abs(min(biases)), max(biases)) * 0.01
            )
            ax_bottom.text(
                bar.get_x() + bar.get_width() / 2,
                y_pos,
    ...
    plt.tight_layout()
    plt.savefig(
        os.path.join(save_dir, "uncertainty_method_comparison.png"),
        dpi=300,
        bbox_inches="tight",
    )
    plt.close(fig)
    ...
    print(
        f"✅ Uncertainty method comparison saved to {save_dir}/uncertainty_method_comparison.png"
    )
...
def plot_summary_uncertainty_scaling(
    ...
    # Aggregate parameter uncertainties using nan-aware operations
    param_uncertainties = np.array(
        scaling_results.get("parameter_uncertainties", [])
    )  # [n_counts, n_params]
    if param_uncertainties.size == 0:
        raise ValueError("No parameter uncertainties found in scaling_results")

    # replace non-finite with nan for safety
    param_uncertainties = np.array(param_uncertainties, dtype=float)
    param_uncertainties[~np.isfinite(param_uncertainties)] = np.nan

    if aggregation_method == "mean":
        agg_param_uncertainty = np.nanmean(param_uncertainties, axis=1)
    elif aggregation_method == "median":
        agg_param_uncertainty = np.nanmedian(param_uncertainties, axis=1)
    elif aggregation_method == "max":
        agg_param_uncertainty = np.nanmax(param_uncertainties, axis=1)
    elif aggregation_method == "rms":
        agg_param_uncertainty = np.sqrt(np.nanmean(param_uncertainties**2, axis=1))
    else:
        agg_param_uncertainty = np.nanmean(param_uncertainties, axis=1)  # fallback

    # Aggregate function uncertainties (build matrix with NaNs where data missing)
    function_uncertainties = scaling_results.get("function_uncertainties", {})
    if function_uncertainties:
        func_names = list(function_uncertainties.keys())
        # each entry may be shorter/longer; build matrix with shape [n_counts, n_functions]
        n_counts = len(event_counts)
        func_matrix = np.full((n_counts, len(func_names)), np.nan, dtype=float)
        for j, fname in enumerate(func_names):
            arr = np.array(function_uncertainties.get(fname, []), dtype=float)
            arr[~np.isfinite(arr)] = np.nan
            L = min(n_counts, arr.shape[0])
            if L > 0:
                func_matrix[:L, j] = arr[:L]

        if aggregation_method == "mean":
            agg_func_uncertainty = np.nanmean(func_matrix, axis=1)
        elif aggregation_method == "median":
            agg_func_uncertainty = np.nanmedian(func_matrix, axis=1)
        elif aggregation_method == "max":
            agg_func_uncertainty = np.nanmax(func_matrix, axis=1)
        elif aggregation_method == "rms":
            agg_func_uncertainty = np.sqrt(np.nanmean(func_matrix**2, axis=1))
        else:
            agg_func_uncertainty = np.nanmean(func_matrix, axis=1)

        # If the aggregated result is all NaN, set to None for downstream logic
        if not np.any(np.isfinite(agg_func_uncertainty)):
    ...
    if include_theoretical_comparison:
        # Normalize to first data point (guard against NaN)
        if np.isfinite(agg_param_uncertainty[0]):
            param_theoretical = agg_param_uncertainty[0] * np.sqrt(
                event_counts[0] / event_counts
            )
        else:
            param_theoretical = None

        if agg_func_uncertainty is not None and np.isfinite(agg_func_uncertainty[0]):
            func_theoretical = agg_func_uncertainty[0] * np.sqrt(
                event_counts[0] / event_counts
            )
        else:
            func_theoretical = None
    ...
    ax.loglog(
        event_counts,
        agg_param_uncertainty,
        "o-",
        color="royalblue",
        linewidth=3,
        markersize=8,
        label=f"{aggregation_method.title()} Parameter Uncertainty",
    )

    if agg_func_uncertainty is not None:
        ax.loglog(
            event_counts,
            agg_func_uncertainty,
            "s-",
            color="darkorange",
            linewidth=3,
            markersize=8,
            label=f"{aggregation_method.title()} Function Uncertainty",
        )
    ...
    if include_theoretical_comparison:
        ax.loglog(
            ...
            alpha=0.7,
            linewidth=2,
            label=r"Parameter $\propto 1/\sqrt{N}$",
        )
        if agg_func_uncertainty is not None:
            ax.loglog(
                event_counts,
    ...
    if include_theoretical_comparison:
        ...
        # Parameter scaling analysis (guarded)
        finite_param_mask = np.isfinite(agg_param_uncertainty) & (
            agg_param_uncertainty > 0
        )
        if np.sum(finite_param_mask) >= 2:
            log_param_unc = np.log(agg_param_uncertainty[finite_param_mask])
            x_for_fit = log_counts[finite_param_mask]
            try:
                param_slope, param_intercept = np.polyfit(x_for_fit, log_param_unc, 1)
                param_r2 = np.corrcoef(x_for_fit, log_param_unc)[0, 1] ** 2
            except Exception:
                param_slope, param_intercept, param_r2 = np.nan, np.nan, np.nan
        else:
            param_slope, param_r2 = np.nan, np.nan

        # Function scaling analysis (guarded)
        if agg_func_uncertainty is not None:
            finite_func_mask = np.isfinite(agg_func_uncertainty) & (
                agg_func_uncertainty > 0
            )
            if np.sum(finite_func_mask) >= 2:
                try:
                    log_func_unc = np.log(agg_func_uncertainty[finite_func_mask])
                    x_for_fit_f = log_counts[finite_func_mask]
                    func_slope, func_intercept = np.polyfit(
                        x_for_fit_f, log_func_unc, 1
                    )
                    func_r2 = np.corrcoef(x_for_fit_f, log_func_unc)[0, 1] ** 2
                except Exception:
        ...
        param_consistency = max(
            0, 1 - abs(param_slope + 0.5) / 0.5
        )  # How close to -0.5
        if not np.isnan(func_slope):
    ...
    plt.tight_layout()
    plt.savefig(
        os.path.join(save_dir, "uncertainty_scaling_summary.png"),
        dpi=300,
        bbox_inches="tight",
    )
    plt.close(fig)
    ...
    ax.set_ylabel("Average Uncertainty", fontsize=14)
    ax.set_title(
        "Uncertainty Quantification Consistency (Linear Scale):\nUncertainty Decreases with Increasing Data",
        fontsize=16,
        pad=20,
    )
    ax.grid(True, alpha=0.3)
    ...
    plt.tight_layout()
    plt.savefig(
        os.path.join(save_dir, "uncertainty_scaling_linear.png"),
        dpi=300,
        bbox_inches="tight",
    )
    plt.close(fig)
    ...
    with open(consistency_path, "w") as f:
        ...
        f.write("=" * 50 + "\n\n")
        f.write(
            "This analysis quantifies how well the uncertainty quantification method\n"
        )
        f.write(
            "follows expected statistical scaling laws, demonstrating consistency.\n\n"
        )
        ...
        if include_theoretical_comparison:
            ...
            f.write(
                f"Consistency score: {param_consistency:.3f} (0=poor, 1=perfect)\n\n"
            )
            ...
            if not np.isnan(func_slope):
                ...
                f.write(
                    f"Consistency score: {func_consistency:.3f} (0=poor, 1=perfect)\n\n"
                )
    ...
    if include_theoretical_comparison:
        ...
        if overall_consistency > 0.8:
            print(
                "   ✅ EXCELLENT: Uncertainty scaling follows expected statistical behavior"
            )
        elif overall_consistency > 0.6:
            print("   ⚠️  GOOD: Uncertainty scaling is mostly consistent")
        else:
            print(
                "   ❌ POOR: Uncertainty scaling deviates significantly from expected behavior"
            )
